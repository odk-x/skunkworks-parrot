...
public class ServerDatabaseCommunicator {
    ...
    private static final List<String> NOTIFICATIONS_TABLE_COLUMNS_LIST = Arrays.asList("NotificationId",
            "NotificationTitle", "NotificationMessage", "GroupId", "NotificationType", "NotificationTime",
            "ResponseList");
     ...
     *
     * @param username the user name to use for initializing SyncClient
     * @param password the password to use for initializing SyncClient
     * @throws IOException   Input errors while calling SyncClient methods
     ...
     *
     * @param notification the Notification object to upload
    ...
    public String uploadNotification(Notification notification) throws JSONException, IOException {
        ...
        List<String> columnValues = Arrays.asList(rowId, notification.getTitle(), notification.getMessage(),
                notification.getGroup_id(), notification.getType(), String.valueOf(notification.getDate()),
                responseList);
        ...
        if (notification.getAttachmentPath() != null && !notification.getAttachmentPath().equals("")) {
            syncClient.putFileForRow(SERVER_URL, APP_ID, NOTIFICATIONS_TABLE_ID, schemaETag, rowId,
                    notification.getAttachmentPath(), RELATIVE_PATH_FOR_ATTACHMENT);
        }
     ...
     *
     * @param group The Group object to upload
     * @return String
    ...
    private boolean isGroupPresent(Group group) throws JSONException {
        ...
        for (Group value : groupArrayList) {
            if (value.getName().equals(group.getName())) {
                present = true;
                break;
            }
        }
    ...
    private void addNotificationToGroup(@Nonnull String groupId, @Nonnull String notificationId) throws IOException, JSONException {
        ...
        if (!temp.equals("")) temp += ",";
        temp += notificationId;
        map.put("NotificationsList", temp);
        ...
        syncClient.updateRowsUsingBulkUpload(SERVER_URL, APP_ID, GROUPS_TABLE_ID, schemaETag, dataETag, rowArrayList,
                1);
    }
    ...
    private void checkTables() throws JSONException, IOException {
        ...
        if (!tablesList.contains(NOTIFICATIONS_TABLE_ID))
            createTable(NOTIFICATIONS_TABLE_ID, NOTIFICATIONS_TABLE_COLUMNS_LIST);
     ...
     *
     * @param group group object whose notification list is required
     * @throws IOException   Due to input errors while calling SyncClient methods
     * @throws JSONException Due to JSON errors while parsing the data
     */
     ...
     *
     * @param notificationId Id for a Notification
     * @throws IOException   Due to input errors while calling SyncClient methods
     * @throws JSONException Due to JSON errors while parsing the data
     */
    public Notification getNotification(@Nonnull String notificationId) throws IOException, JSONException {
        ...
        return getNotificationFromJSON(notificationArray);
    }

    /**
     * Returns a array list of responses for given notification
     *
     * @param notification Notification object whose response list is required
     * @throws IOException   Due to input errors while calling SyncClient methods
     * @throws JSONException Due to JSON errors while parsing the data
     */
    public ArrayList<Response> getResponsesList(Notification notification) throws JSONException {
    ...
    public ArrayList<Response> getResponses() throws JSONException {
        ...
        for (int i = 0; i < responsesJsonArray.size(); i++) {
            responseArrayList.add(getResponseFromJSON(responsesJsonArray.getJSONObject(i).getJSONArray(
                    "orderedColumns")));
        }
        return responseArrayList;
    }
    ...
    private Row getRowFromJSON(JSONObject jsonObject) throws JSONException {
        ...
        if (jsonObject.get("dataETagAtModification") != null)
            row.setDataETagAtModification(jsonObject.get("dataETagAtModification").toString());
        if (jsonObject.get("savepointCreator") != null)
            row.setSavepointCreator(jsonObject.get("savepointCreator").toString());
        ...
        if (jsonObject.get("savepointTimestamp") != null)
            row.setSavepointTimestamp(jsonObject.get("savepointTimestamp").toString());
        return row;
    }
